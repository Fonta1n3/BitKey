...
class HDChildViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func addButtonView() {
        ...
        let x = (self.view.center.x - (17.5)) - (self.view.frame.width / 4 - (17.5)) - 5
        let spendButton = UIButton(frame: CGRect(x: x, y: 25, width: 35, height: 35))
        ...
        let historyButton = UIButton(frame: CGRect(x: (self.view.center.x - (17.5)), y: 25, width: 35, height: 35))
        ...
        let addX = ((self.view.center.x - (17.5)) + (self.view.frame.width / 4 - (17.5)) - 5)
        let addSaveButton = UIButton(frame: CGRect(x: addX, y: 25, width: 35, height: 35))
        ...
        let saveX = addSaveButton.center.x - (addSaveButton.frame.width / 2)
        let saveLabel = UILabel(frame: CGRect(x: saveX, y: 61, width: addSaveButton.frame.width, height: 12))
       ...
       let exportButton = UIButton(frame: CGRect(x: self.view.frame.maxX - 45, y: 25, width: 35, height: 35))
        ...
        let exportX = exportButton.center.x - (exportButton.frame.width / 2)
        let exportLabel = UILabel(frame: CGRect(x: exportX, y: 61, width: exportButton.frame.width, height: 12))
    ...
    func checkBalance(address: String, index: Int, network: String, type: String) {
        ...
        switch true {
        case address.hasPrefix("1") || address.hasPrefix("3"):
            url = NSURL(string: "https://blockchain.info/rawaddr/\(address)")
            print("getLegacyBalance")
            getLegacyBalance()
        case address.hasPrefix("m") || address.hasPrefix("2") || address.hasPrefix("n"):
            url = NSURL(string: "https://testnet.blockchain.info/rawaddr/\(address)")
            print("getTestNetBalance")
            getTestNetBalance()
        case address.hasPrefix("b"):
            url = NSURL(string: "https://api.blockchair.com/bitcoin/dashboards/address/\(address)")
            print("getSegwitBalance")
            getSegwitBalance()
        case address.hasPrefix("t"):
        ...
        default:
            displayAlert(viewController: self, title: "Error", message: "That is not a Bitcoin Address")
        }
    }
