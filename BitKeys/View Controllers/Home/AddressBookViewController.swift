...
class AddressBookViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, UIGestureRecognizerDelegate {
   ...
   func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldReceive touch: UITouch) -> Bool {
    ...
    func checkSettings() {
        ...
        if !hotMode && !coldMode && !legacyMode && !segwitMode && !mainnetMode && !testnetMode {
        ...
        let currencies = ["BTC":"bitcoin", "SAT":"satoshi", "USD":"dollar", "GBP":"pounds", "EUR":"euro"]
        for (key, value) in currencies {
            
            if let xe = checkTransactionSettingsForKey(keyValue: value) as? Bool {
                if xe {
                    self.currency = key
                }
            }
    ...
    func firstTimeHere() {
        ...
        if KeychainWrapper.standard.object(forKey: "firstTime") == nil {
            ...
            //Removes old BIP39 password from User Defaults ands saves to Keychain
            ...
            let context = appDelegate.persistentContainer.viewContext
            let defaultSettingsFetchRequest = NSFetchRequest<NSManagedObject>(entityName: "Settings")
                
            do {
                    
                let results = try context.fetch(defaultSettingsFetchRequest) as [NSManagedObject]
    ...
    func ensureBackwardsCompatibility() {
        ...
        DispatchQueue.main.async {
            ...
            let alert = UIAlertController(title: "Things have changed", message: "We have done a big upgrade to the wallet, don't worry your Bitcoin are safe, we will automatically add your hot wallet to your new \"Address Book\". Now you can have many wallets saved at the same time, all accesible here on your home screen. Tap OK to proceed.", preferredStyle: UIAlertControllerStyle.alert)
    ...
    func addButtonView() {
        ...
        let x = (self.view.center.x - (17.5)) - (self.view.frame.width / 2.25 - (17.5)) - 5
        let frame = CGRect(x: x, y: 28, width: 35, height: 35)
        let createInvoiceButton =  UIButton(frame: frame)
        ...
        let spendButton = UIButton(frame: CGRect(x: (self.view.center.x - (17.5)) - (self.view.frame.width / 4 - (17.5)) - 5, y: 28, width: 35, height: 35))
        ...
        historyButton.frame = CGRect(x: (self.view.center.x - (17.5)), y: 28, width: 35, height: 35)
        ...
        let first = self.view.center.x - 17.5
        let second = (self.view.frame.width / 4) - 22.5
        let editButtonX = first + second - 5
        let editButton = UIButton(frame: CGRect(x: editButtonX, y: 28, width: 35, height: 35))
        ...
        let exportButton = UIButton(frame: CGRect(x: (self.view.center.x - (17.5)) + (self.view.frame.width / 2.25 - (17.5)) - 5, y: 28, width: 35, height: 35))
   ...
   @objc func add() {
    ...
    func convertBTCtoCurrency(btcAmount: String, exchangeRate: Double) -> String {
        ...
        var convertedAmount = ""
        let noCommas = btcAmount.replacingOccurrences(of: ",", with: "")
        let btcDouble = Double(noCommas)!
                        ...
                        do {
                            ...
                            if let exchangeCheck = jsonQuoteResult["bpi"] as? NSDictionary {
                                ...
                                if let xeCheck = exchangeCheck[currencyToShow] as? NSDictionary {
                                    ...
                                    print("xeCheck = \(xeCheck)")
                                    
                                        if let rateCheck = xeCheck["rate_float"] as? Double {
                                            
                                            print("rateCheck = \(rateCheck)")
   ...
   func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt: IndexPath) -> [UITableViewRowAction]? {
        ...
        let hideBalance = UITableViewRowAction(style: .normal, title: "Hide Balance") { action, index in
            print("hideBalance button tapped")
            
            let indexPath = IndexPath(item: 0, section: editActionsForRowAt.section)
            let cell = tableView.cellForRow(at: indexPath)
            let balanceLabel = cell?.viewWithTag(1) as! UILabel
            let currencyLabel = cell?.viewWithTag(3) as! UILabel
            DispatchQueue.main.async {
        ...
        hideBalance.backgroundColor = .blue
        ...
        return [delete, edit, hideBalance]
    }
    ...
    func addHomeScreen() {
        ...
        DispatchQueue.main.async {
            ...
            let priceBX = ((self.view.center.x - 20) - (self.view.frame.width / 2.5 - 20) - 5)
            let priceButton = UIButton(frame: CGRect(x: priceBX, y: 10, width: 30, height: 30))
            if modelName == "iPhone X" {
                priceButton.frame = CGRect(x: priceBX, y: 10, width: 30, height: 30)
            }
            ...
            let tBBX = ((self.view.center.x - 20) - (self.view.frame.width / 6 - 20) - 5)
            let toolboxButton = UIButton(frame: CGRect(x: tBBX, y: 10, width: 35, height: 35))
            
            if modelName == "iPhone X" {
                toolboxButton.frame = CGRect(x: tBBX, y: 10, width: 35, height: 35)
            }
            ...
            self.lockButton = UIButton(frame: CGRect(x: (self.view.center.x - 17.5) + (self.view.frame.width / 6 - (17.5)) + 5, y: 10, width: 35, height: 35))
            if modelName == "iPhone X" {
                self.lockButton.frame = CGRect(x: (self.view.center.x - 17.5) + ((self.view.frame.width / 6) - 17.5) + 5, y: 10, width: 35, height: 35)
            }
            ...
            let addressBookButton = UIButton(frame: CGRect(x: (self.view.center.x - (17.5)) + ((self.view.frame.width / 2.5) - 17.5) + 5, y: 10, width: 35, height: 35))
            if modelName == "iPhone X" {
                addressBookButton.frame = CGRect(x: (self.view.center.x - 17.5) + ((self.view.frame.width / 2.5) - 17.5) + 5, y: 10, width: 35, height: 35)
            }
    ...
    func getLockingPassword() {
        ...
        DispatchQueue.main.async {
            ...
            let alert = UIAlertController(title: "Protect your wallet by setting a password that locks and unlocks it.", message: "Please do not forget this password, you will need it to use the app, this is not mandatory but is highly recommended.", preferredStyle: .alert)
            alert.addTextField { (textField1) in
            ...
            alert.addAction(UIAlertAction(title: NSLocalizedString("Add", comment: ""), style: .destructive, handler: { (action) in
                ...
                confirmationAlert.addAction(UIAlertAction(title: NSLocalizedString("Confirm", comment: ""), style: .destructive, handler: { (action) in
                    ...
                    if firstPassword == secondPassword {
                        ...
                        displayAlert(viewController: self, title: "Error", message: "Passwords did not match, tap the lock button to try again.")
                    }
                }))
